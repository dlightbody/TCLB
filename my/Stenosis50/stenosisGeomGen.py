...
domainMinPt = Vector3(-2.5,-2.5,-2.5)
domainMaxPt = Vector3(2.5,1.25,2.5)

#define the packing volume

# readfile and add points
with open('Stenosis_50_wo_cylinder_points.txt','r') as f:
    data = f.readlines()
    points = []
    for line in data:
        point = line.split()
...
# Add cells
TriMesh = TriPatchSet()
with open('Stenosis_50_wo_cylinder_cells.txt','r') as c:
    data = c.readlines()
    for line in data:
        cell = line.split()
        cell = map(int,cell)
...
# readfile and add points
with open('Stenosis_50_wo_cylinder_tag_points.txt','r') as f:
    data = f.readlines()
    points = []
    for line in data:
        point = line.split()
...
with open('Stenosis_50_wo_cylinder_tag_cells.txt','r') as c:
    data = c.readlines()
    for line in data:
        cell = line.split()
        cell = map(int,cell)
...
mntable.write("Stenosis_50_low_res.geo",1)

#write the geometry in VTK format for visualisation purposes:
mntable.write("Stenosis_50_low_res.vtu",2)
